...
const QString Worker::genNewFileName(const QString &filePath)
{
    ...
    while (fileInfo.exists()) {
        ...
        {
            probeFile = dir.absoluteFilePath(basename + "_" + QString::number(postfixCount) + "." + suffix);
        }
...
void Worker::doWork()
{
    ...
    if (!m_inDir.exists())
    {
        emit finished(WorkerResult::Error, "Directory does not exist: '" + m_inDir.absolutePath() + "'");
        return;
    }

    if (!m_outDir.exists())
    {
        emit finished(WorkerResult::Error, "Directory does not exist: '" + m_outDir.absolutePath () + "'");
        return;
    }
    ...
    foreach (const QString &fileName, inFileNames) {
        ...
        if (!inFile.open(inMode)) {
             emit finished(WorkerResult::Error, "Failed to open input file '" + inFile.fileName() + "': " + inFile.errorString());
            return;
        }
        ...
        if (!encryptSelf)
        {
            if (!outFile.open(QIODevice::WriteOnly)) {
                emit finished(WorkerResult::Error, "Failed to open output file '" + outFile.fileName() + "': " + outFile.errorString());
                return;
            }
        }
        ...
        while (!inFile.atEnd()) {
            ...
            if (writtenBytes != bufferSize) {
                emit finished(WorkerResult::Error, "Not all data was written to the file '" + outFile.fileName() + "': " + outFile.errorString());
                return;
            }
